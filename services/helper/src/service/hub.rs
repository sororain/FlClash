...
mod tests {
    ...
    async fn ping_returns_running_helper_path_for_verified_core() {
        let response = ping_response(Ok(PathBuf::from("SororainHelperService.exe")));
    ...
    async fn start_rejects_a_caller_supplied_core_argument() {
        let response = warp::test::request()
            ...
            .body(
                r#"{"address":"\\\\.\\pipe\\SororainCore_0123456789abcdef0123456789abcdef","sessionId":"0123456789abcdef0123456789abcdef","path":"attacker.exe"}"#,
            )
            .reply(&routes())
            .await;
    ...
    async fn start_releases_the_managed_core_before_rejecting_an_unverified_core() {
        ...
        *MANAGED_CORE.lock().unwrap() = Some(ManagedCore {
        ...
        let response = warp::test::request()
            ...
            .path("/start")
            .json(&StartParams {
                address: r"\\.\pipe\SororainCore_0123456789abcdef0123456789abcdef".to_string(),
    ...
    fn verifies_core_sha256_in_all_build_modes() {
        let path =
            std::env::temp_dir().join(format!("sororain-helper-core-sha256-{}", std::process::id()));
    ...
    fn only_accepts_random_core_pipe_namespace() {
        ...
        assert!(!is_allowed_core_pipe(r"\\.\pipe\SororainCore"));
        assert!(!is_allowed_core_pipe(
            r"\\.\pipe\Other_0123456789abcdef0123456789abcdef"
        ));
        assert!(!is_allowed_core_pipe(
            r"\\.\pipe\SororainCore_0123456789abcdef"
        ));
        assert!(!is_allowed_core_pipe(
            r"\\.\pipe\SororainCore_0123456789abcdef0123456789abcdeg"
        ));
        assert!(!is_allowed_core_pipe(
            r"\\.\pipe\SororainCore_ABCDEF0123456789abcdef0123456789"
        ));
    }
}
