...
import 'package:fl_clash/iqoo/services/subscription_service.dart';
...
class StoreAction extends _$StoreAction {
  ...
  Future handleClear() async {
    final profileIds = ref.read(profilesProvider).map((item) => item.id).toSet();
    ...
    if (await providersDir.exists()) {
      await for (final entity in providersDir.list(followLinks: false)) {
        ...
        final profileId = int.tryParse(name);
...
class ProfilesAction extends _$ProfilesAction {
  ...
  Future<void> syncSubscriptionWithRetry({int maxAttempts = 3}) async {
